...
document.querySelectorAll('.acc-item').forEach(item=>{const btn=item.querySelector('.menu-btn');const panel=item.querySelector('.content-section');setMaxHeight(panel,false);btn.addEventListener('click',()=>{const isOpen=panel.classList.toggle('open');btn.classList.toggle('active',isOpen);btn.setAttribute('aria-expanded',isOpen?'true':'false');setMaxHeight(panel,isOpen);saveState();if(isOpen)scrollToPanel(panel);});});
window.addEventListener('resize',()=>{document.querySelectorAll('.content-section.open').forEach(panel=>{panel.style.maxHeight=panel.scrollHeight+'px';});});
restoreState();